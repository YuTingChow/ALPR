...
def apply_tesseract(x, img, im0, f, minPLsize=160):
    import pytesseract
    texts = []
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                im = cv2.resize(cutout, (minPLsize, int(minPLsize*cutout.shape[0]/cutout.shape[1])))  # BGR
                im = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
                im = cv2.medianBlur(im, 3)
                config = ("-c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNPQRSTUWXYZ -l eng --oem 1 --psm 13")
                text = pytesseract.image_to_string(im, config=config)
                k=1
                if len(text)==0:
                    return x, texts
                while text[:k].isalnum() :
                    k+=1
                k-=1
                texts.append(text[:k])
                f.write(str(int(a[1]))+','+str(int(a[3]))+','+str(int(a[0]))+','+str(int(a[2]))+','+texts[-1]+'\n')
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                half = d.device.type != 'cpu'  # half precision only supported on CUDA
                imgsz = check_img_size(160, s=model.stride.max())  # check img_size
                # cutout = cutout.half() if half else cutout.float()
                # im = cv2.resize(cutout, (160, int(160*cutout.shape[0]/cutout.shape[1])))  # BGR

                im = letterbox(cutout, new_shape=imgsz)[0]
                cutout0 = im.copy()
                cv2.imwrite('test%i.jpg' % j, im)
                
                im = im[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x160x160
                im = np.ascontiguousarray(im, dtype=np.float32)  # uint8 to float32
                im = torch.from_numpy(im).to(d.device)
                if half:
                    model.half()  # to FP16
                ...
                pred = non_max_suppression(pred, 0.4, 0.4, classes=None, agnostic=False)
                # print('second stage prediction output ', len(pred), ' ', pred.shape)
                # ims.append(im)
                text = ''
                for k, e in enumerate(pred):
                    if e is not None and len(e):
                        for l, f in enumerate(e):
                            cutChar = cutout0[int(max(f[1]-3, 0)):int(f[3]+3), int(max(f[0]-3, 0)):int(f[2]+3)]
                            cutChar = letterbox(cutChar, new_shape=100, color = (0,0,0))[0]
                            cutChar = cv2.cvtColor(cutChar, cv2.COLOR_BGR2GRAY)
                            cv2.imwrite('PL%i-%i.jpg' % (k, l), cutChar)
                            # cutChar = cv2.resize(cutChar, (50, 100), interpolation=cv2.INTER_CUBIC)
                            cutChar = cv2.GaussianBlur(cutChar, (5,5),0.5)
                            config = ("-c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNPQRSTUWXYZ -l eng --oem 1 --psm 10")
                            regT = pytesseract.image_to_string(cutChar, lang='eng', config = config)
                            text = text + regT[0:1]
                    print(text) 
            # Apply NMS
            
            # print('After non_max_suppression ',pred, pred[0].shape)
            x = pred
            # pred_cls2 = model(torch.Tensor(ims).to(d.device)).argmax(1)  # classifier prediction
            # x[i] = x[i][pred_cls1 == pred_cls2]  # retain matching class detections
